-- Create adaptive chart data function
-- Returns raw data points when count <= max_points (default 200)
-- Only aggregates into time buckets when there are more points than the threshold
-- This maximizes data resolution for issuers with fewer trades
--
-- Always prepends a "carry-forward" point at the window start:
--   - Uses the most recent price_history row BEFORE p_start_time if one exists
--   - Otherwise falls back to issuer_trading.base_price at created_at
-- This ensures even single-trade timeframes show movement from the prior price.
...
BEGIN
    ...
    THEN
        -- No previous history — use the base price at creation time
    ...
    ELSE
        ...
        -- Use bucketed aggregation (same logic as get_ohlc_data)
        RETURN QUERY
        WITH bucketed AS (
            SELECT
...
GRANT EXECUTE ON FUNCTION get_chart_data_adaptive(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, INT) TO public;

-- Add comment
COMMENT ON FUNCTION get_chart_data_adaptive IS 'Adaptive chart data: prepends the base_price origin point, then returns raw points when <= max_points (default 200), otherwise aggregates into ~max_points time buckets.';
